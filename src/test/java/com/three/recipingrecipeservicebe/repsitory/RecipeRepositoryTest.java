...
public class RecipeRepositoryTest {
    ...
    void getLatestPage1List() {
        ...
        Page<Recipe> results = recipeRepository.findPagedByCreatedAtDesc(pageable);
        ...
        // 결과의 첫 번째, 마지막 항목 타이틀 검증
        List<String> titles = results.getContent().stream()
                .map(Recipe::getTitle)
                .toList();

        assertThat(titles.get(0)).isEqualTo("제목20"); // 최신순이면 가장 최근 제목
        assertThat(titles.get(titles.size() - 1)).isEqualTo("제목9");
    }
    ...
    void getLatestPage2List() {
        ...
        List<Recipe> content = results.getContent();

        assertThat(content).hasSize(8); // 전체 20개 중 앞 12개 제외한 나머지
        assertThat(content.get(0).getTitle()).isEqualTo("제목8");
        assertThat(content.get(7).getTitle()).isEqualTo("제목1");
    }
